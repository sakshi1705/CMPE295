...
describe("# Project APIs", () => {
    ...
    const newProject = {
        title: 'Bidding Node.js backend',
        summary: 'Testing the APIs',
                ...
                .then(() => {
                    return request.get(apiBase + "/project")
                        ...
                        .expect(res => {
                            ...
                            let item = res.body[0];
                            item.should.have.property('id').to.equal(newProject.id);
                            item.should.have.property('name').to.equal(newProject.name);
                            item.should.have.property('tier').to.equal(newProject.tier);
                        });
                })
        })
